...
class ParseableDatum:
    ...
    Attributes:
        ...
        metadata: Dict[(np.ndarray, int, float), str] is actual data combined with a string that can be used to id that
        data for later use
    """
    references: Dict[str, str] = field(default_factory=dict)
    metadata: Dict[any, str] = field(default=None)
    ...
    def __post_init__(self):
        ...
        for k, v in self.metadata.items():
            if not (isinstance(k, (np.ndarray, int, float)) and isinstance(v, str)):
                raise TypeError(
